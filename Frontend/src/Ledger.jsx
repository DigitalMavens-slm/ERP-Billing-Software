  ...
  <div className="max-w-6xl mx-auto p-4 md:p-6">
    ...
    <div className="relative mb-6">
      ...
      {showList && suggestions.length > 0 && (
        <ul
          className="absolute z-20 w-full bg-white shadow-lg border
                     rounded-lg max-h-60 overflow-y-auto mt-1"
        >
  ...
  <table className="w-full text-left text-sm md:text-base">
    ...
    <thead>
      <tr className="bg-gray-100 text-gray-700 font-medium">
        ...
        <th className="p-3 hidden sm:table-cell">Invoice No</th>
        <th className="p-3 hidden md:table-cell">Debit (₹)</th>
        <th className="p-3 hidden md:table-cell">Credit (₹)</th>
        ...
        ledger.map((row, i) => (
          ...
          >
            ...
            <td className="p-3 hidden md:table-cell">
              {typeof row.debit === "number" ? row.debit.toFixed(2) : "-"}
            </td>

            <td className="p-3 hidden md:table-cell">
              {typeof row.credit === "number" ? row.credit.toFixed(2) : "-"}
            </td>
